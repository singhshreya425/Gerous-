...
function solution(length,weightArr,profitArr,maxWeight) {
    let maxProfit=0;
      for(let i=0;i<length;i++){
          for(let j=i+1;j<length;j++){
              if(maxWeight>=weightArr[i]+weightArr[j]){
  ...
  function main() {
      ...
      var weightArr=readLine().split(" ").map(Number)
      var profitArr=readLine().split(" ").map(Number)
      var maxWeight=parseInt(readLine())
  
      var res = solution(length,weightArr,profitArr,maxWeight);
      console.log(res);
  }
